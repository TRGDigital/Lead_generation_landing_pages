...
export function GetMoreEnquiries() {
  return (
    // Full-bleed accent-yellow band so this section reads as its own moment on the page
    // (the section above is a white outlined box; the one below is the dark ink band).
    <section className="relative overflow-hidden bg-brand-accent px-6 py-24">
      <Star className="absolute right-8 top-10 hidden h-20 w-20 rotate-12 text-brand-ink/15 lg:block" />
      <Dots className="absolute bottom-10 left-6 hidden h-24 w-24 text-brand-ink/15 lg:block" />
      <Squiggle className="absolute -right-8 bottom-24 hidden h-7 w-48 rotate-6 text-brand-pop lg:block" />

      <div className="relative mx-auto max-w-6xl">
        <div className="max-w-2xl">
          <span className="inline-block rounded-full bg-brand-ink px-4 py-1.5 font-display text-sm font-bold uppercase tracking-widest text-brand-accent">
            Get more enquiries
          </span>
          <h2 className="mt-5 font-display text-3xl font-bold uppercase leading-tight tracking-tight text-brand-ink sm:text-4xl">
        ...
        <div className="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {CAPABILITIES.map(({ icon: Icon, title, body }) => (
            <div
              key={title}
              className="group rounded-2xl border-2 border-brand-ink bg-white p-5 shadow-[4px_4px_0_0_#2a2620] transition-all hover:-translate-y-0.5 hover:bg-brand-pop hover:shadow-[6px_6px_0_0_#2a2620]"
            >
